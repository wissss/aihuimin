...
define(["service/kernel/jobrank", 'lib/mymodal', 'lib/mytable'], function (jobrank, mymodal, mytable) {
    ...
    function registerEvent(table) {
        //给查询函数注册点击事件
        $("#kernel_jobrank_userrole_save").click(function () {
           var allData = table.bootstrapTable('getData');
           
            jobrank.saveUserRole(allData, function (result) {
        ...
        //给重置查询函数注册点击事件
        $("#kernel_jobrank_userrole_reset").click(function () {
            table.bootstrapTable('refresh');
        });
        //初始化界面给checkbox 选中
    ...
    //初始化查询条件
    function initQueryCondition(jobrankdata) {
        way.set("kernel_jobrank_user_role.userrole_querydata", jobrankdata);
        return 1;
    }
    //返回查询条件给表格组件
    function queryParams(src) {
        src["jobRankId"] = way.get("kernel_jobrank_user_role.userrole_querydata.id");
        return src;
    }
